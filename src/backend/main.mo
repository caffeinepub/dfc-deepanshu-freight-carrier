...
// Attach migration logic using the "with" clause
...
actor {
  ...
  // Google API Key Storage (persistent)
  ...
  type LoginHistoryEntry = {
    identifier : Text; // Email or mobile used for login
    clientId : Text;
    loginTime : Time.Time;
    ipAddress : ?Text;
  };

  // Persisted login history state (Now persistent!)
  ...
  public shared ({ caller }) func setShipmentCoordinates(
    trackingID : Text,
    latitude : Float,
    longitude : Float,
    status : Text,
    ...
    switch (shipments.get(trackingID)) {
      case (null) {
        Runtime.trap("Shipment not found with tracking ID: " # trackingID);
      };
  ...
  public shared ({ caller }) func updateShipmentCoordinates(
    trackingID : Text,
    latitude : Float,
    longitude : Float,
    adminToken : Text,
  ...
  public shared ({ caller }) func setGoogleApiKey(apiKey : Text, adminToken : Text) : async () {
    if (not isValidSession(adminToken)) {
      Runtime.trap("Unauthorized: Invalid admin token");
    };
    ...
    switch (googleApiKey) {
      ...
      case (?currentApiKey) {
        if (Text.equal(currentApiKey, apiKey)) {
          Runtime.trap("The provided Google API key is already set and unchanged.");
        };
        googleApiKey := ?apiKey;
      };
    };
  ...
  public query ({ caller }) func isGoogleApiKeyConfigured() : async Bool {
    googleApiKey != null;
  };
  ...
  public shared ({ caller }) func authenticateClient(emailOrMobile : Text, password : Text, ipAddress : ?Text) : async ?Text {
    ...
    switch (matchedAccountKey, matchedAccount) {
      case (?accountKey, ?account) {
        ...
        // Store login history entry only for successful login
  ...
  public shared ({ caller }) func verifyOtpAndAuthenticate(phoneNumber : Text, otp : Text, ipAddress : ?Text) : async ?Text {
    ...
    // Only store in login history if matching account exists
    switch (matchedAccountKey, matchedAccount) {
      case (?accountKey, ?account) {
        ...
        // Store login history entry for successful OTP login
        let historyEntry : LoginHistoryEntry = {
  ...
  // New Query to Fetch Login History (Admin only)
  public query ({ caller }) func getLoginHistory(adminToken : Text) : async [LoginHistoryEntry] {
    ...
    let entriesArray = loginHistory.toArray();
    entriesArray.reverse().map(func((_, entry)) { entry });
  };
};
