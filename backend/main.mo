...
actor {
  ...
  // Persistent admin password allows changing it in the future
  var adminPassword : Text = "Jatin2580";
  ...
  // Auto-repair logic for client accounts
  ...
  // Bulk repair function for missing linkedPrincipals
  ...
  // Admin session and authentication APIs
  public type AdminSession = {
    token : Text;
    expiration : Time.Time;
  };

  // adminLogin must be a shared (update) call because it mutates state:
  // it stores the session token and assigns the admin role.
  ...
  // validateAdminSession must be shared because isValidAdminSession mutates state
  // (removes expired tokens and refreshes timestamps).
  ...
  public shared ({ caller }) func adminLogout(token : Text) : async () {
    // Allow logout if the caller is admin OR if they hold a valid admin session token.
    // This handles the case where the caller may not yet have the role persisted
    // but holds a valid session token.
    let hasValidSession = isValidAdminSession(token);
    let isAdminCaller = AccessControl.isAdmin(accessControlState, caller);
    if (not isAdminCaller and not hasValidSession) {
      Runtime.trap("Unauthorized: Only admins can logout");
    };
    adminSessionTokens.remove(token);
  };
