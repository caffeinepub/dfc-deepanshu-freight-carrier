...
function AppContent() {
  ...
  const handleTrack = async () => {
    ...
    try {
      if (!actor) {
        setTrackingResult('Service temporarily unavailable. Please try again later.');
        return;
      }

      // Check if trackShipment method exists
      if (typeof (actor as ExtendedActor).trackShipment !== 'function') {
        setTrackingResult('Tracking service is currently unavailable. Please contact DFC office.');
        return;
      }
      ...
      if (shipment) {
        ...
        setTrackingResult('Invalid Tracking ID. Please verify and try again or contact DFC office.');
      }
    } catch (error: any) {
      ...
      if (errorMessage.includes('not a function') || errorMessage.includes('has no method')) {
        ...
        setTrackingResult('Network error. Please check your connection and try again.');
      } else {
        setTrackingResult('Error tracking shipment. Please try again or contact DFC office.');
      }
    } finally {
      setIsTracking(false);
    }
              ...
              <Card className="bg-neutral-900 border-neutral-800">
                <CardContent className="pt-6">
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      <h3 className="text-gold font-semibold text-lg mb-2">Address</h3>
            ...
            <p className="text-sm">
              ...
              <a
                href={`https://caffeine.ai/?utm_source=Caffeine-footer&utm_medium=referral&utm_content=${encodeURIComponent(
                  typeof window !== 'undefined' ? window.location.hostname : 'dfc-transport'
                )}`}
                target="_blank"
